...
- `functions/api/proxy/[[path]].js`：Cloudflare Pages Functions 代理
- `favicon.svg`：站点图标
...
本项目推荐部署到 Cloudflare Pages。

原因：

- GitHub Pages 只能托管静态文件，无法运行后端代理；
- 部分 OpenAI-compatible API 服务商限制浏览器跨域请求；
- Cloudflare Pages Functions 可以提供同源代理接口 `/api/proxy/*`；
- 前端统一请求 `/api/proxy/chat/completions`，由 Pages Function 转发到用户填写的 BaseURL。
...
BaseURL 应填写完整 OpenAI-compatible API 根路径，通常包含 `/v1`。
...
启用 Cloudflare Pages Functions 代理后，API Key 会经过你部署的 Cloudflare Pages Function。
...
本地测试 Cloudflare Pages Functions 可使用：
...
GitHub Pages 只能托管静态文件，无法运行 Cloudflare Pages Functions，因此代理接口 `/api/proxy/*` 将不可用。
